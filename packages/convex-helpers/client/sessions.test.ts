...
describe("ConvexSessionClient", () => {
  ...
  beforeEach(() => {
    ...
    sessionClient = new ConvexSessionClient(mockClient as unknown as ConvexClient, sessionId);
  });
  ...
  it("should inject sessionId into query args", async () => {
    ...
    expect(mockClient.query).toHaveBeenCalledWith(query, { ...args, sessionId });
  ...
  it("should inject sessionId into mutation args", async () => {
    ...
    expect(mockClient.mutation).toHaveBeenCalledWith(mutation, { ...args, sessionId });
  ...
  it("should inject sessionId into action args", async () => {
    ...
    expect(mockClient.action).toHaveBeenCalledWith(action, { ...args, sessionId });
  ...
  it("should allow changing the sessionId", async () => {
    ...
    expect(mockClient.query).toHaveBeenCalledWith(query, { sessionId: newSessionId });
