...
async function runWithLocalBackend(command, backendUrl) {
  if (process.env.CONVEX_LOCAL_BACKEND === undefined) {
    console.error("Please set environment variable CONVEX_LOCAL_BACKEND first")
    process.exit(1)
  }
  ...
  console.log("Backend running! Logs can be found in $CONVEX_LOCAL_BACKEND/convex-local-backend.log")
